module m_fuvNode
  ...
  !                .      .    .                                       .
  ! subprogram:    module m_fuvNode
  !   prgmmr:      c cantrall 
  !     date:      2021-07-18
  !
  ! abstract: class-module of obs-type fuvNode (far ultraviolet)
  !
  ...
  ! module interface:
    ...
    type,extends(obsNode):: fuvNode
       !type(fuv_ob_type),pointer :: llpoint => NULL()
       type(obs_diag), pointer :: diags => NULL()
       real(r_kind)    :: res    =0._r_kind    !  fuv residual
       real(r_kind)    :: err2   =0._r_kind    !  fuv error squared
       ...
       real(r_kind)    :: wij(4) =0._r_kind    !  horizontal interpolation weight
    ...
    public:: fuvNode_typecast
    public:: fuvNode_nextcast
          interface fuvNode_typecast; module procedure typecast_ ; end interface
          interface fuvNode_nextcast; module procedure nextcast_ ; end interface
  
    public:: fuvNode_appendto
          interface fuvNode_appendto; module procedure appendto_ ; end interface
  
    character(len=*),parameter:: MYNAME="m_fuvNode"
  
#include "myassert.H"
#include "mytrace.H"
  contains
  function typecast_(aNode) result(ptr_)
  !-- cast a class(obsNode) to a type(fuvNode)
    use m_obsNode, only: obsNode
    implicit none
    type(fuvNode),pointer:: ptr_
  ...
  !-- cast an obsNode_next(obsNode) to a type(fuvNode)
  ...
  !-- append aNode to linked-list oLL
    ...
    implicit none
    type(fuvNode),pointer,intent(in):: aNode
  ...
  subroutine obsNode_init_(aNode)
    use gridmod, only: nsig
    implicit none
    class(fuvNode),intent(out):: aNode
  
    character(len=*),parameter:: myname_=MYNAME//'::obsNode_init_'
  _ENTRY_(myname_)
    allocate(aNode%ij(12,nsig)                                  )
  _EXIT_(myname_)
    return
  ...
  subroutine obsNode_clean_(aNode)
    implicit none
    class(fuvNode),intent(inout):: aNode
