...
! Added by cantrall 07/19/2021
  if(trim(filename) == 'fuvncdf')return
...
! Added by cantrall 07/29/2020
    use goldinfo, only: ntype_gold, typeuse_gold, diag_gold
...
! Added by cantrall 07/29/2020       
         else if(ditype(i) == 'fuv') then
          if(diag_gold .and. .not. reduce_diag)minuse=-2
          do j=1,ntype_gold
             if(typeuse_gold(j) > minuse) nuse=.true.
          end do
       else
          nuse=.true.
       end if
...
! Added by cantrall 07/29/2020
!         Process GOLD far ultraviolet observations               
          else if(ditype(i) == 'fuv')then
               if(obstype == 'gold')then
                  !call read_gold()
                  string='READ_GOLD'
               endif
