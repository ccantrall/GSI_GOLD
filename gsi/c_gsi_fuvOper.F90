...
!                .      .    .                                       .
! subprogram:	 module gsi_fuvOper
!
! abstract: an obOper extension for fuvNode type
!
! attributes:
...
! module interface:
  ...
  implicit none
  public:: fuvOper      ! data stracture

  type,extends(obOper):: fuvOper
  contains
    procedure,nopass:: mytype
    procedure,nopass:: nodeMold
    procedure:: setup_
    procedure:: intjo1_ ! procedure is needed for variational assimilation 
    procedure:: stpjo1_ ! procedure is needed for variational assimilation
  end type fuvOper

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_fuvOper'
  type(fuvNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    use m_rhs  , only: iwork => i_fuv
    
    use obsmod, only: write_diag
    use goldinfo, only: diag_gold
    use jfunc , only: jiter

    use mpeu_util, only: die
    implicit none
    class(fuvOper ), intent(inout):: self
    ...
    diagsave  = write_diag(jiter) .and. diag_gold
...
! Commented out these lines because intfuvmod not defined for fuv   
    !use intfuvmod, only: intjo => intfuvlay ! No TL and ADJ for FUV yet (07/20/2021)
...
!
    ...
    !call intjo(headNode, rval,sval) ! No TL and ADJ for FUV yet (07/20/2021)
    headNode => null()

  end subroutine intjo1_

  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
! Commented out these lines because stpfuvmod not defined for fuv
    !    use stpfuvmod, only: stpjo => stpfuvlay ! No TL and ADJ for FUV yet (07/20/2021)
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
   ...
   !----------------------------------------
    ...
    headNode => obsLList_headNode(self%obsLL(ibin))
    !call stpjo(headNode,dval,xval,pbcjo(:),sges,nstep) ! No TL and ADJ for FUV yet (07/20/2021)
    headNode => null()
  end subroutine stpjo1_

end module gsi_fuvOper
